...
$(BASH_PREFIX)/%: $(BASH_DIR_SRC)/%
	@install -m 0700 -d -v $(dir $@)
	@install -m 0600 -v $< $@

bash_install: $(addprefix $(BASH_PREFIX)/, $(patsubst ${BASH_DIR_SRC}/%,%,${BASH_FILES}))
	@install -m 0700 -d -v ${BASH_PREFIX}

insert_bashrc_loader :
	@grep -q '^while read -r BASHRC' ${HOME}/.bashrc ||\
		echo 'while read -r BASHRC ; do source "$$BASHRC" ; done < <(find "${BASH_PREFIX}" -type f -name "*.bash")' >> ${HOME}/.bashrc
